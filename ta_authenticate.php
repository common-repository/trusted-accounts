...
class TrustedAccountsAuthenticate
{

    public static function authorizationendpoint($branding = true)
    {
        $user_email = "";
        ...
        $authorizationUrl = $auth_code_url . "?email=" . urlencode($user_email)
                                            . "&client_id=" . urlencode($client_id)
                                            . "&scope=" . urlencode($scope) 
                                            . "&redirect_uri=" . urlencode(home_url()) 
                                            . "&response_type=code&state=" . urlencode($state);

        // Conditionally remove branding
        if (!$branding) {
            $authorizationUrl .= "&branding=false";
        }
